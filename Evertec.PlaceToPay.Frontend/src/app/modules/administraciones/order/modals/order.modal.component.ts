...
import {
  ...
  StatusEnum,
  StatusResult,
} from "../../status/services/status.object";
...
export class OrderModalComponent extends BaseModalComponent<Order> implements OnInit {
  ...
  validateStatusOrder() {
    if (
      this.objEntidad.statusId !== undefined &&
      this.objEntidad.statusId !== StatusEnum.payed
    ) {
      this.isPayment = true;
      this.isCreate = false;
    } else {
  ...
  onPayment(): void {
    this.paymentService
      .makePayment(this.objEntidad.orderId)
      .subscribe((res: PaymentResult) => {
        if (res.success) {
